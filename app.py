...
# -------------------------------------------------------------
# 1. SYNTHETIC DATA ENGINE (Replace with live Broker API feeds)
# -------------------------------------------------------------
@st.cache_data(ttl=2)
def get_market_data():
    now = datetime.now()
    ...
    # Trend Scalper Impulse & Double Dots
...
# -------------------------------------------------------------
# 2. TOP DASHBOARD KPI METRICS
# -------------------------------------------------------------
st.title("⚡ Quant OptionScalp Dashboard")

col1, col2, col3, col4, col5 = st.columns(5)
with col1:
    st.markdown('<div class="metric-box status-bearish">PUT POC: ₹{:.2f}</div>'.format(put_poc), unsafe_allow_html=True)
with col2:
    st.markdown('<div class="metric-box status-bullish">CALL POC: ₹{:.2f}</div>'.format(call_poc), unsafe_allow_html=True)
with col3:
    st.markdown('<div class="metric-box status-wait">STRADDLE TLOC: ₹{:.2f}</div>'.format(straddle_tloc), unsafe_allow_html=True)
with col4:
...
# -------------------------------------------------------------
# 3. MULTI-PANE CHART SETUP (Plotly)
# -------------------------------------------------------------
fig = make_subplots(
    rows=3, cols=1, 
    shared_xaxes=True, 
...
# Top Pane: Put (Red) vs Call (Green) + Strike POCs
...
# Overlay TS Double Dots on Put Options
...
# Middle Pane: Straddle vs Straddle VWAP & TLOC
...
# Bottom Pane: Oscillator / SMI Momentum
...
# -------------------------------------------------------------
# 4. LIVE ORDER FLOW & POWER HISTORY MATRIX
# -------------------------------------------------------------
